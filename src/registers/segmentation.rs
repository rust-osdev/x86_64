...
mod kani_harnesses {
    use super::*;
    #[kani::proof]
    fn kani_harness_389_new() {
        let index: u16 = kani::any();
        let rpl: PrivilegeLevel = kani::any();
        SegmentSelector::new(index, rpl);
    }

    #[kani::proof]
    fn kani_harness_390_index() {
        let callee: SegmentSelector = kani::any();
        callee.index();
    }

    #[kani::proof]
    fn kani_harness_391_rpl() {
        let callee: SegmentSelector = kani::any();
        callee.rpl();
    }

    #[kani::proof]
    fn kani_harness_392_set_rpl() {
        let rpl: PrivilegeLevel = kani::any();
        let mut callee: SegmentSelector = kani::any();
        callee.set_rpl(rpl);
    }
}
