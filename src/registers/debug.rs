...
mod kani_harnesses {
    use super::*;
    #[kani::proof]
    fn kani_harness_151_new() {
        let n: u8 = kani::any();
        DebugAddressRegisterNumber::new(n);
    }

    #[kani::proof]
    fn kani_harness_152_get() {
        let callee: DebugAddressRegisterNumber = kani::any();
        callee.get();
    }

    #[kani::proof]
    fn kani_harness_189_trap() {
        let n: DebugAddressRegisterNumber = kani::any();
        Dr6Flags::trap(n);
    }

    #[kani::proof]
    fn kani_harness_226_local_breakpoint_enable() {
        let n: DebugAddressRegisterNumber = kani::any();
        Dr7Flags::local_breakpoint_enable(n);
    }

    #[kani::proof]
    fn kani_harness_227_global_breakpoint_enable() {
        let n: DebugAddressRegisterNumber = kani::any();
        Dr7Flags::global_breakpoint_enable(n);
    }

    #[kani::proof]
    fn kani_harness_228_from_bits() {
        let bits: u64 = kani::any();
        BreakpointCondition::from_bits(bits);
    }

    #[kani::proof]
    fn kani_harness_229_bit_range() {
        let n: DebugAddressRegisterNumber = kani::any();
        BreakpointCondition::bit_range(n);
    }

    #[kani::proof]
    fn kani_harness_230_new() {
        let size: usize = kani::any();
        BreakpointSize::new(size);
    }

    #[kani::proof]
    fn kani_harness_231_from_bits() {
        let bits: u64 = kani::any();
        BreakpointSize::from_bits(bits);
    }

    #[kani::proof]
    fn kani_harness_232_bit_range() {
        let n: DebugAddressRegisterNumber = kani::any();
        BreakpointSize::bit_range(n);
    }

    #[kani::proof]
    fn kani_harness_233_valid_bits() {
        Dr7Value::valid_bits();
    }

    #[kani::proof]
    fn kani_harness_234_from_bits() {
        let bits: u64 = kani::any();
        Dr7Value::from_bits(bits);
    }

    #[kani::proof]
    fn kani_harness_235_from_bits_truncate() {
        let bits: u64 = kani::any();
        Dr7Value::from_bits_truncate(bits);
    }

    #[kani::proof]
    unsafe fn kani_harness_236_from_bits_unchecked() {
        let bits: u64 = kani::any();
        unsafe {
            Dr7Value::from_bits_unchecked(bits);
        }
    }

    #[kani::proof]
    fn kani_harness_237_bits() {
        let callee: Dr7Value = kani::any();
        callee.bits();
    }

    #[kani::proof]
    fn kani_harness_238_flags() {
        let callee: Dr7Value = kani::any();
        callee.flags();
    }

    #[kani::proof]
    fn kani_harness_239_condition() {
        let n: DebugAddressRegisterNumber = kani::any();
        let callee: Dr7Value = kani::any();
        callee.condition(n);
    }
    ...
    #[kani::proof]
    fn kani_harness_240_set_condition() {
        let n: DebugAddressRegisterNumber = kani::any();
        let condition: BreakpointCondition = kani::any();
        let mut callee: Dr7Value = kani::any();
        callee.set_condition(n, condition);
    }

    #[kani::proof]
    fn kani_harness_241_size() {
        let n: DebugAddressRegisterNumber = kani::any();
        let callee: Dr7Value = kani::any();
        callee.size(n);
    }
    ...
    #[kani::proof]
    fn kani_harness_242_set_size() {
        let n: DebugAddressRegisterNumber = kani::any();
        let size: BreakpointSize = kani::any();
        let mut callee: Dr7Value = kani::any();
        callee.set_size(n, size);
    }
}
