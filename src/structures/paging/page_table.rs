...
mod kani_harnesses {
    use super::*;
    #[kani::proof]
    fn kani_harness_561_new() {
        PageTableEntry::new();
    }

    #[kani::proof]
    fn kani_harness_562_is_unused() {
        let callee: PageTableEntry = kani::any();
        callee.is_unused();
    }

    #[kani::proof]
    fn kani_harness_563_set_unused() {
        let mut callee: PageTableEntry = kani::any();
        callee.set_unused();
    }

    #[kani::proof]
    fn kani_harness_564_flags() {
        let callee: PageTableEntry = kani::any();
        callee.flags();
    }

    #[kani::proof]
    fn kani_harness_565_addr() {
        let callee: PageTableEntry = kani::any();
        callee.addr();
    }

    #[kani::proof]
    fn kani_harness_566_frame() {
        let callee: PageTableEntry = kani::any();
        callee.frame();
    }

    #[kani::proof]
    fn kani_harness_567_default() {
        PageTableEntry::default();
    }

    #[kani::proof]
    fn kani_harness_604_new() {
        PageTable::new();
    }

    #[kani::proof]
    fn kani_harness_605_default() {
        PageTable::default();
    }

    #[kani::proof]
    fn kani_harness_606_new() {
        let index: u16 = kani::any();
        PageTableIndex::new(index);
    }

    #[kani::proof]
    fn kani_harness_607_new_truncate() {
        let index: u16 = kani::any();
        PageTableIndex::new_truncate(index);
    }

    #[kani::proof]
    fn kani_harness_608_into_u64() {
        let callee: PageTableIndex = kani::any();
        callee.into_u64();
    }

    #[kani::proof]
    fn kani_harness_609_steps_between() {
        let start: PageTableIndex = kani::any();
        let end: PageTableIndex = kani::any();
        PageTableIndex::steps_between(&start, &end);
    }

    #[kani::proof]
    fn kani_harness_610_forward_checked() {
        let start: PageTableIndex = kani::any();
        let count: usize = kani::any();
        PageTableIndex::forward_checked(start, count);
    }

    #[kani::proof]
    fn kani_harness_611_backward_checked() {
        let start: PageTableIndex = kani::any();
        let count: usize = kani::any();
        PageTableIndex::backward_checked(start, count);
    }

    #[kani::proof]
    fn kani_harness_612_new() {
        let offset: u16 = kani::any();
        PageOffset::new(offset);
    }

    #[kani::proof]
    fn kani_harness_613_new_truncate() {
        let offset: u16 = kani::any();
        PageOffset::new_truncate(offset);
    }

    #[kani::proof]
    fn kani_harness_614_next_lower_level() {
        let callee: PageTableLevel = kani::any();
        callee.next_lower_level();
    }

    #[kani::proof]
    fn kani_harness_615_next_higher_level() {
        let callee: PageTableLevel = kani::any();
        callee.next_higher_level();
    }

    #[kani::proof]
    fn kani_harness_616_table_address_space_alignment() {
        let callee: PageTableLevel = kani::any();
        callee.table_address_space_alignment();
    }

    #[kani::proof]
    fn kani_harness_617_entry_address_space_alignment() {
        let callee: PageTableLevel = kani::any();
        callee.entry_address_space_alignment();
    }
}
