...
use structures::paging::{Page, PhysFrame, VirtAddr};
use ux::u9;
...
impl<'a> RecursivePageTable<'a> {
        ...
        A: FnMut(PhysFrame),
    {
        ...
        if p4_entry.is_unused() {
            return Err(UnmapError::PageNotMapped(page));
        }
        if p4_entry.flags().contains(Flags::HUGE_PAGE) {
            return Err(UnmapError::EntryWithInvalidFlagsPresent(
                page,
                p4_entry.clone(),
            ));
        }
        ...
        Err(UnmapError::EntryWithInvalidFlagsPresent(
            page,
            p1_entry.clone(),
        ))
    }
