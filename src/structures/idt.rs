...
mod kani_harnesses {
    use super::*;
    #[kani::proof]
    fn kani_harness_477_new() {
        InterruptDescriptorTable::new();
    }

    #[kani::proof]
    fn kani_harness_478_default() {
        InterruptDescriptorTable::default();
    }

    /*
    #[kani::proof]
    fn kani_harness_479_eq() {
        let other: Entry<T/#0> = kani::any();
        let callee: Entry = kani::any();
        callee.eq(&other);
    }
    ...
    */
    #[kani::proof]
    fn kani_harness_480_missing() {
        Entry::<i16>::missing();
    }

    #[kani::proof]
    fn kani_harness_481_handler_addr() {
        let callee: Entry<i16> = kani::any();
        callee.handler_addr();
    }

    #[kani::proof]
    fn kani_harness_482_minimal() {
        EntryOptions::minimal();
    }

    #[kani::proof]
    unsafe fn kani_harness_483_set_code_selector() {
        let cs: SegmentSelector = kani::any();
        let mut callee: EntryOptions = kani::any();
        unsafe {
            callee.set_code_selector(cs);

        }
    }

    #[kani::proof]
    fn kani_harness_484_set_present() {
        let present: bool = kani::any();
        let mut callee: EntryOptions = kani::any();
        callee.set_present(present);
    }

    #[kani::proof]
    fn kani_harness_485_present() {
        let callee: EntryOptions = kani::any();
        callee.present();
    }

    #[kani::proof]
    fn kani_harness_486_disable_interrupts() {
        let disable: bool = kani::any();
        let mut callee: EntryOptions = kani::any();
        callee.disable_interrupts(disable);
    }

    #[kani::proof]
    fn kani_harness_487_set_privilege_level() {
        let dpl: PrivilegeLevel = kani::any();
        let mut callee: EntryOptions = kani::any();
        callee.set_privilege_level(dpl);
    }

    #[kani::proof]
    fn kani_harness_488_privilege_level() {
        let callee: EntryOptions = kani::any();
        callee.privilege_level();
    }

    #[kani::proof]
    unsafe fn kani_harness_489_set_stack_index() {
        let index: u16 = kani::any();
        let mut callee: EntryOptions = kani::any();
        unsafe {
            callee.set_stack_index(index);
        }
    }

    #[kani::proof]
    fn kani_harness_490_stack_index() {
        let callee: EntryOptions = kani::any();
        callee.stack_index();
    }

    #[kani::proof]
    fn kani_harness_527_new() {
        let value: u64 = kani::any();
        SelectorErrorCode::new(value);
    }

    #[kani::proof]
    fn kani_harness_528_new_truncate() {
        let value: u64 = kani::any();
        SelectorErrorCode::new_truncate(value);
    }

    #[kani::proof]
    fn kani_harness_529_external() {
        let callee: SelectorErrorCode = kani::any();
        callee.external();
    }

    #[kani::proof]
    fn kani_harness_530_descriptor_table() {
        let callee: SelectorErrorCode = kani::any();
        callee.descriptor_table();
    }

    #[kani::proof]
    fn kani_harness_531_index() {
        let callee: SelectorErrorCode = kani::any();
        callee.index();
    }

    #[kani::proof]
    fn kani_harness_532_is_null() {
        let callee: SelectorErrorCode = kani::any();
        callee.is_null();
    }
}
