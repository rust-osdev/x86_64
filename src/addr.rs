...
mod kani_harnesses {
    ...
    #[kani::proof]
    fn kani_harness_2_new() {
        let addr: u64 = kani::any();
        VirtAddr::new(addr);
    }

    #[kani::proof]
    fn kani_harness_3_try_new() {
        let addr: u64 = kani::any();
        VirtAddr::try_new(addr);
    }

    #[kani::proof]
    fn kani_harness_4_new_truncate() {
        let addr: u64 = kani::any();
        VirtAddr::new_truncate(addr);
    }

    #[kani::proof]
    unsafe fn kani_harness_5_new_unsafe() {
        let addr: u64 = kani::any();
        unsafe {
    ...
    #[kani::proof]
    fn kani_harness_6_zero() {
        VirtAddr::zero();
    }

    #[kani::proof]
    fn kani_harness_7_as_u64() {
        let callee: VirtAddr = kani::any();
        callee.as_u64();
    }
    ...
    #[kani::proof]
    fn kani_harness_10_is_null() {
        let callee: VirtAddr = kani::any();
        callee.is_null();
    }

    #[kani::proof]
    fn kani_harness_11_align_down_u64() {
        let align: u64 = kani::any();
        let callee: VirtAddr = kani::any();
        callee.align_down_u64(align);
    }

    #[kani::proof]
    fn kani_harness_12_is_aligned_u64() {
        let align: u64 = kani::any();
        let callee: VirtAddr = kani::any();
        callee.is_aligned_u64(align);
    }

    #[kani::proof]
    fn kani_harness_13_page_offset() {
        let callee: VirtAddr = kani::any();
        callee.page_offset();
    }

    #[kani::proof]
    fn kani_harness_14_p1_index() {
        let callee: VirtAddr = kani::any();
        callee.p1_index();
    }

    #[kani::proof]
    fn kani_harness_15_p2_index() {
        let callee: VirtAddr = kani::any();
        callee.p2_index();
    }

    #[kani::proof]
    fn kani_harness_16_p3_index() {
        let callee: VirtAddr = kani::any();
        callee.p3_index();
    }

    #[kani::proof]
    fn kani_harness_17_p4_index() {
        let callee: VirtAddr = kani::any();
        callee.p4_index();
    }

    #[kani::proof]
    fn kani_harness_18_page_table_index() {
        let level: PageTableLevel = kani::any();
        let callee: VirtAddr = kani::any();
        callee.page_table_index(level);
    }

    #[kani::proof]
    fn kani_harness_19_steps_between_impl() {
        let start: VirtAddr = kani::any();
        let end: VirtAddr = kani::any();
        VirtAddr::steps_between_impl(&start, &end);
    }

    #[kani::proof]
    fn kani_harness_20_forward_checked_impl() {
        let start: VirtAddr = kani::any();
        let count: usize = kani::any();
        VirtAddr::forward_checked_impl(start, count);
    }

    #[kani::proof]
    fn kani_harness_21_add() {
    ...
    #[kani::proof]
    fn kani_harness_22_add_assign() {
        let rhs: u64 = kani::any();
        let mut callee: VirtAddr = kani::any();
        callee.add_assign(rhs);
    }

    #[kani::proof]
    fn kani_harness_23_sub() {
        let rhs: u64 = kani::any();
        let callee: VirtAddr = kani::any();
        callee.sub(rhs);
    }
    ...
    #[kani::proof]
    fn kani_harness_24_sub_assign() {
        let rhs: u64 = kani::any();
        let mut callee: VirtAddr = kani::any();
        callee.sub_assign(rhs);
    }

    #[kani::proof]
    fn kani_harness_25_sub() {
        let rhs: VirtAddr = kani::any();
        let callee: VirtAddr = kani::any();
        callee.sub(rhs);
    }

    #[kani::proof]
    fn kani_harness_26_steps_between() {
        let start: VirtAddr = kani::any();
        let end: VirtAddr = kani::any();
        VirtAddr::steps_between(&start, &end);
    }

    #[kani::proof]
    fn kani_harness_27_forward_checked() {
        let start: VirtAddr = kani::any();
        let count: usize = kani::any();
        VirtAddr::forward_checked(start, count);
    }

    #[kani::proof]
    fn kani_harness_28_backward_checked() {
        let start: VirtAddr = kani::any();
        let count: usize = kani::any();
        VirtAddr::backward_checked(start, count);
    }

    #[kani::proof]
    fn kani_harness_29_new() {
        let addr: u64 = kani::any();
        PhysAddr::new(addr);
    }

    #[kani::proof]
    fn kani_harness_30_new_truncate() {
        let addr: u64 = kani::any();
        PhysAddr::new_truncate(addr);
    }

    #[kani::proof]
    unsafe fn kani_harness_31_new_unsafe() {
        let addr: u64 = kani::any();
        unsafe {
    ...
    #[kani::proof]
    fn kani_harness_32_try_new() {
        let addr: u64 = kani::any();
        PhysAddr::try_new(addr);
    }

    #[kani::proof]
    fn kani_harness_33_zero() {
        PhysAddr::zero();
    }

    #[kani::proof]
    fn kani_harness_34_as_u64() {
        let callee: PhysAddr = kani::any();
        callee.as_u64();
    }

    #[kani::proof]
    fn kani_harness_35_is_null() {
        let callee: PhysAddr = kani::any();
        callee.is_null();
    }

    #[kani::proof]
    fn kani_harness_36_align_down_u64() {
        let align: u64 = kani::any();
        let callee: PhysAddr = kani::any();
        callee.align_down_u64(align);
    }
    ...
    #[kani::proof]
    fn kani_harness_37_is_aligned_u64() {
        let align: u64 = kani::any();
        let callee: PhysAddr = kani::any();
        callee.is_aligned_u64(align);
    }
    ...
    #[kani::proof]
    fn kani_harness_39_add_assign() {
        let rhs: u64 = kani::any();
        let mut callee: PhysAddr = kani::any();
        callee.add_assign(rhs);
    }

    #[kani::proof]
    fn kani_harness_40_sub() {
        let rhs: u64 = kani::any();
        let callee: PhysAddr = kani::any();
        callee.sub(rhs);
    }
    ...
    #[kani::proof]
    fn kani_harness_42_sub() {
        let rhs: PhysAddr = kani::any();
        let callee: PhysAddr = kani::any();
        callee.sub(rhs);
    }
}
